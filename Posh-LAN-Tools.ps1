...
If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]'Administrator')) {
    ...
    sleep 1
    if ($stage -eq 'y'){
        Start-Process PowerShell.exe -ArgumentList ("-NoP -Ep Bypass -C `$stage = 'y'; irm https://raw.githubusercontent.com/beigew0rm/Posh-LAN/main/Posh-LAN-Tools.ps1 | iex") -Verb RunAs
    }
    else{
